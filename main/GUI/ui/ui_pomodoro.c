...
static void update_chart(float temp, int humi) {
    ...
    // Температура в десятых долях градуса (диапазон 0..500 для 0..50°C)
...
void ui_pomodoro_show_window(void) {
    ...
    lv_obj_t *right_box = lv_obj_create(pomodoro_scr);
    ...
    lv_obj_t *sep_line = lv_line_create(pomodoro_scr);
    ...
    // --- Ось времени ---
    lv_obj_t *time_axis = lv_line_create(pomodoro_scr);
    ...
    // --- Ось влажности ---
    lv_obj_t *humi_axis = lv_line_create(pomodoro_scr);
    ...
    lv_obj_t *humi_label = lv_label_create(pomodoro_scr);
    ...
    // --- Ось температуры ---
    lv_obj_t *temp_axis = lv_line_create(pomodoro_scr);
    ...
    lv_obj_t *temp_label = lv_label_create(pomodoro_scr);
    ...
    lv_obj_set_style_size(chart, 0, 0, LV_PART_INDICATOR);     // ширина = 0, высота = 0
    ...
    // Тестовые данные (синусоиды для наглядности)
    ...
    ESP_LOGI(TAG, "Pomodoro window created: points hidden, secondary axis range 0-500, blue separator line at y=220.");
}